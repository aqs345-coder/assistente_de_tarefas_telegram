...
def add_reminder(user_id, chat_id, text, start_date, end_date, time):
    with get_connection() as conn:
        cur = conn.execute(
            """
            INSERT INTO reminders (user_id, chat_id, text, start_date, end_date, time, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (user_id, chat_id, text, start_date, end_date, time, date.today().isoformat()),
        )
